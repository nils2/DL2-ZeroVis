    ...
    def __init__(self, tokenizer, args: FrozenArgs = FrozenArgs()):
        ...
        if self.args.shared_emb_dim is None:
            if len(self.args.text_emb_layers) == 1:
                if (self.args.text_emb_layers[0] in [-1, self.lm.config.num_hidden_layers
            ...
            for layer_idx in self.args.text_emb_layers:
                if (layer_idx == -1 or layer_idx
                        == self.lm.config.num_hidden_layers) and ('bert' not in opt_version):
                    in_dim = self.lm.config.word_embed_proj_dim

                    text_fc = [
                        nn.Linear(in_dim, out_dim), nn.Dropout(self.args.text_embed_dropout_prob)
                    ]
            ...
            for i in range(max_len):
                if 'opt' in self.opt_version:
                    ...
                    if i == 0:
                        ...
                        output = self.lm(input_ids=out[:, -1:],
                                         use_cache=True,
                ...
                if self.args.shared_emb_dim is not None:
                    for idx, fc_layer in zip(self.args.text_emb_layers, self.text_hidden_fcs):
                        hidden_states.append(fc_layer(
                            output.hidden_states[idx]))  # (N, seq_len, 2048)
                else:
                ...
                if self.retrieval_token_idx != -1 and self.retrieval_token_idx is not None:
                    if i < min_word_tokens:
                        ...
                        logits[:, self.
                               retrieval_token_idx] = logits[:, self.
                                                             retrieval_token_idx] * ret_scale_factor
                ...
                elif (self.tokenizer.eos_token_id and
...
class Fromage(nn.Module):
    ...
    def __init__(self,
        ...
        self.visual_embs = torch.load(visual_embs)
    ...
    def __call__(self,
        ...
        if generate:
            return self.model.generate(images,
                                       num_words,
    ...
    def generate_for_images_and_texts(self,
                                      prompts: List,
                                      num_words: int = 0,
                                      ret_scale_factor: float = 1.0,
                                      top_p: float = 1.0,
    ...
    """
        ...
        for i, p in enumerate(prompts):
            if type(p) == list:
                ...
                input_embs.append(
                    visual_emb.to(device=self.model.logit_scale.device,
                                  dtype=self.model.logit_scale.dtype))

            elif type(p) == str:
                text_ids = self.model.tokenizer(p, add_special_tokens=True,
                ...
                input_embs.append(
                    p.to(device=self.model.logit_scale.device, dtype=self.model.logit_scale.dtype))
            else:
                raise ValueError(
        ...
        if num_words == 0:
            ...
            outputs = self.model.lm(inputs_embeds=input_embs,
                                    use_cache=False,
                                    output_hidden_states=True)
        ...
        all_ret_idx = [
            i for i, x in enumerate(generated_ids[0, :] == self.model.retrieval_token_idx) if x
        ][:max_num_rets]
...
def load_fromage(model_dir: str,
    ...
    model = Fromage(tokenizer,
                    args,
                    path_array=path_array,
                    emb_matrix=emb_matrix,
                    visual_embs=visual_emb_path)
    model = model.eval()
