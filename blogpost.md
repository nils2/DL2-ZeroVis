...
In the era of evergrowing language models, a key challenge to obtaining models that can reason about the textual as well as visual world, lies in the lack of multimodal grounding. Language models (LMs) are trained on massive text corpora and demonstrate impressive capabilities such as generating human-like dialogue, answering complex questions, and learning new tasks from very few examples. Despite this, they are generally incapable of incorporating visual cues. This limits the ability to perform tasks that require visual reasoning.

Multimodal grounding offers a solution to this. Grounding is the general concept of enriching the embedding space of one modality with the knowledge of another. In the vision-language case, this can be done through merging or aligning the spaces of the vision and text modality. This has several benefits. For one, textual embeddings can be informed better using visual grouding (Shahmohammadi et al., 2022). Furthermore, it removes the models' limitations in visual reasoning.

The paper "Grounding Language Models to Images for Multimodal Generation" facilitates multimodal grounding by introducing FROMAGe (Frozen Retrieval Over Multimodal Data for Autoregressive Generation), a foundational framework which extends the semantic capabilities of LMs to the text-and-image realm in a very efficient manner.

FROMAGe (Koh et al., 2023) employs a pretrained language model and a pretrained visual encoder, both kept frozen, to ground language models visually. This grounding is achieved by appending a linear mapping layer to align the pretrained models, and training the combined model with a multitask objective for image captioning and image-text retrieval. 

For the language model, the architecture uses an autoregressive pretrained LM $v_\theta$, with frozen parameters $\theta$, which predicts the next token conditioned on the previous tokens in a sequence. $v_\theta$ is pre-trained to maximize the log likelihood of the input token sequence. Token sequences are extracted from input text using a BPE tokenizer. Based on input tokens, $v_\theta$ outputs text embeddings of hidden dimensionality $d$. For the vision model, a pre-trained visual backbone $v_\phi$ with frozen parameters $\phi$ is used, which produces visual embeddings of dimensionality $m$.

Alignment between the language and vision model is realized by learning translation parameters to map both embedding spaces to the same dimensionality. The translation parameters are represented by a single linear layer with **$W_c$**. This mapping works either way, depending on the task. On one hand, for image captioning visual embeddings $v_\phi(y) \in \mathbb{R}^{m}$ are mapped using a matrix of shape **$W_c$**$\in \mathbb{R}^{m \times kd}$ to $\mathbb{k \times d}$, representing $k$ vectors (where $k$ is chosen) within the LM's input space, hence $d$. On the other hand, for visual retrieval textual and visual embeddings are mapped to a new dimensionality $q$ using two differently sized matrices. A new retrieval token [RET] is appended to the LM's input. This token will be used to retrieve imags upon inference later. Its last hidden state $h_{\theta}(x_i) \in \mathbb{R}^p$ is multiplied with the matrix **$W_t$**$\in \mathbb{R}^{p \times q}$. Our visual embeddings are mapped using the other matrix, **$W_i$**$\in \mathbb{R}^{m \times q}$. This results in both embeddings existing in the same $q$-dimensional retrieval embedding space.

--clarifying image on mapping--

For the image captioning task the model learns to extract visual embeddings from input, and maps these into the input space of the LM through the mapping layer. It is trained on the maximum likelihood objective to optimize next-token prediction. On the other hand, the text-image retrieval task is trained uses contrastive learning to map the embeddings of the new [RET] token, representing an image to be retrieved, to the corresponding visual embeddings. Resuting image embeddings are evaluated against the ground truth image in the $q$-embedding space. During both of FROMAGe's training processes, the only weights that are updated are those of the linear layers and of the [RET] token, ensuring computation and memory efficiency. The authors of FROMAGe explicitly refrain from fine-tuning either of the models separately because this would adapt the parameter configurations that make these models so powerful in their respective domains. They reason this enables better generalization for in-context learning.

--clarifying image on training--

Koh et al. (2023) highlight several key strengths of FROMAGe as a framework for multimodal generation. Firstly, their model retains the original abilities of the text-only language model, such as text generation, while also acquiring new multimodal dialogue and reasoning abilities. Unlike LM's, input and output can consist of text intertwined with (retrieved) images. Secondly, the model demonstrates strong few-shot multimodal abilities learned from image-caption pairs alone, which is a standout feature as most models require web-scale interleaved image-text data (Alayrac et al., 2022; Aghajanyan et al., 2022). Thirdly, it demonstrates superior text-to-image retrieval sensitivity, especially for long and complex free-form text. Lastly, FROMAGe enhances the existing capabilities of pretrained language models, such as in-context learning, input sensitivity, and dialogue generation, for visually grounded tasks. 

Indeed, the authors display a feat of impressive multimodal dialogue capabilities and zero-shot retrieval. A mention is made of the improvement of resulting retrieval ability with the incluson of more context. It becomes unclear, however, whether this improved retrieval is merely assisted by the LM's world knowledge or whether the model can also reason about multiple visual inputs in connection with eachother. The latter touches upon the open question whether foundational models can perform semantic reasoning on the comparison of different scenes. We wish to continue this line of research by investigating arithmetics on visual inputs.

(or instead: 'in the context of multimodal generation')

### Grounding for Visual Arithmetic Tasks
In the context of deep learning, the concept of arithmetics is not concerned with traditional numerical operations, but rather with finding analogies or relationships between different entities. This idea takes inspiration from the vector arithmetic introduced by Mikolov et al. (2013). In their work, they demonstrated that word embeddings can be manipulated using arithmetic operations to reveal semantic relationships between words. Extending this idea into the visual domain, we can also define visual arithmetic tasks which involve understanding semantic relationships between images, and applying a transformation observed in a pair of images to another. A classic example would be: if "Obama" is to "USA" as "X" is to "Germany", what is "X"? The correct answer would ideally be "Angela Merkel", following the logic of leaders to their respective countries.
...
Explain which llm and vit are used. Explain that the BPE tokenizer is used (what does 'byte-level' mean?).
